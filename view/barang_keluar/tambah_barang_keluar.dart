...
class _tambahBKState extends State<tambahBK> {
  ...
  final String linkT = URL.urltujuanbk.toString();
  ...
  dialogsukses(String pesan) {
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        ...
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => new dataBK()));
      },
  ...
  save() async {
    try {
      final respons =
          await http.post(Uri.parse(URL.urltambahbk), body: {
        "tujuan": Tjuan,
        "ket": ket,
      });
      ...
      String pesan = data['message']?.toString() ?? '';
      print(data);
      if (code == 1) {
        setState(() {
          dialogsukses(pesan);
